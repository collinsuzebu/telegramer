...
class GroupFiles:
	...
	def group_album(self, directory, groupby, custom_title):
		...
		"""
		audio_names = [os.path.join(directory, audio) for audio in os.listdir(directory) if os.path.isfile(os.path.join(directory, audio))]
		...
		for album_name, v in album_names.items():
			...
			if not os.path.isdir(album_dir) and len(v) >= 3 and os.path.basename(album_dir) != 'None':
